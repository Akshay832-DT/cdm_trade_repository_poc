...
def generate_init_file(module_path: List[str], classes: List[str], circular_deps: Set[Tuple[str, str]]) -> str:
    ...
    # First, collect all dependencies and imports
    dependencies = {}
    all_imports = set()
    for class_name in classes:
        ...
        if os.path.exists(class_file):
            with open(class_file, 'r') as f:
                ...
                for import_path, import_name in type_checking_imports:
                    all_imports.add(f"from {import_path} import {import_name}")
                    dependencies[class_name].append(import_name)
                # Find all regular imports after class definition
                regular_imports = re.findall(r'# Import after class definition.*?\n((?:from .*? import .*?\n)+)', class_content, re.DOTALL)
                if regular_imports:
                    for imp_block in regular_imports:
                        for imp in imp_block.strip().split('\n'):
                            if imp:
                                all_imports.add(imp)
    
    # Add all imports before any class imports
    for imp in sorted(all_imports):
        content.append(imp)
    content.append("")
    
    # Add regular imports for classes in this module
    for class_name in classes:
        content.append(f"from {module_name}.{snake_case(class_name)} import {class_name}")
    ...
    # Sort classes based on dependencies, handling circular dependencies
    sorted_classes = []
    visited = set()
    temp_visited = set()
    ...
    def visit(class_name):
        ...
        if class_name in temp_visited:
            # We have a circular dependency, handle it by using ForwardRef
            return
...
def generate_model_file(schema_name: str, schema: CdmSchema, module_path: List[str], module_dir: Path, circular_deps: Set[Tuple[str, str]]):
    ...
    imports = [
        ...
        'from typing import Dict, List, Optional, Any, Union, ForwardRef, TYPE_CHECKING'
    ]
