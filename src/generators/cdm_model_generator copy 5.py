...
def determine_cross_import(current_module: List[str], target_module: List[str], class_name: str, circular_deps: Set[Tuple[str, str]]) -> str:
    ...
    if (tuple(current_module), tuple(target_module)) in circular_deps:
        return f"from typing import ForwardRef\n{class_name} = ForwardRef('{class_name}')"
        
    return f"from {rel_path} import {class_name}"

def determine_property_type(prop_name: str, prop: CdmProperty, class_name: str, module_path: List[str], type_checking_imports: set, circular_deps: Set[Tuple[str, str]]) -> str:
    ...
    if prop.ref:
        ...
        if ref_module:
            # Handle special cases for datetime and other base modules
            if ref_module[0] == "base" and ref_module[1] == "datetime":
                if ref_class.startswith("FieldWithMeta"):
                    type_checking_imports.add(f"from ...metafields import {ref_class}")
                else:
                    type_checking_imports.add(f"from ...base.datetime import {ref_class}")
            elif ref_module[0] == "base":
                type_checking_imports.add(f"from ...base.{'.'.join(ref_module[1:])} import {ref_class}")
            else:
                type_checking_imports.add(determine_cross_import(module_path, ref_module, ref_class, circular_deps))
        else:
    ...
    if prop.is_array:
        if prop_name == "business_center":
            ...
            if items_module:
                # Handle special cases for datetime and other base modules
                if items_module[0] == "base" and items_module[1] == "datetime":
                    if items_class.startswith("FieldWithMeta"):
                        type_checking_imports.add(f"from ...metafields import {items_class}")
                    else:
                        type_checking_imports.add(f"from ...base.datetime import {items_class}")
                elif items_module[0] == "base":
                    type_checking_imports.add(f"from ...base.{'.'.join(items_module[1:])} import {items_class}")
                else:
                    type_checking_imports.add(determine_cross_import(module_path, items_module, items_class, circular_deps))
            else:
    ...
    for module_path, schema_names in module_schemas.items():
        ...
        for i, schema_name in enumerate(schema_names):
            ...
            for prop_name, prop in schema.properties.items():
                ...
                prop_type = determine_property_type(prop_name, prop, class_name, module_path, type_checking_imports, circular_deps)
...
def update_init_file(directory: Path, dependencies: Dict[str, Dict[str, str]], module_schemas: List[str]):
    ...
    def visit(cls):
        ...
        for dep, dep_type in sorted(deps.items(), key=lambda x: ['base', 'enum', 'direct', 'array'].index(x[1]) if x[1] in ['base', 'enum', 'direct', 'array'] else 3):
    ...
    # Import remaining classes in dependency order
    for cls in sorted_classes:
        if not cls.endswith('Enum') and not any(dep_type == 'base' for dep_type in dir_classes[cls].values()):
            snake_name = camel_to_snake(cls)
            content.append(f"from .{snake_name} import {cls}\n")
    
    # Add model_rebuild() calls
    content.append("\n# Rebuild models to handle forward references\n")
    for cls in sorted_classes:
        content.append(f"{cls}.model_rebuild()\n")
