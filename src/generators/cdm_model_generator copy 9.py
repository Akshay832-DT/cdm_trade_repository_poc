...
def determine_property_type(prop_name: str, prop: CdmProperty, class_name: str, module_path: List[str], type_checking_imports: set, regular_imports: set, circular_deps: Set[Tuple[str, str]]) -> str:
    ...
    if prop.ref:
        ...
        if ref_class == "MetaFields" or ref_class.startswith("FieldWithMeta"):
            ...
            return f'"{ref_class}"'
        elif ref_class.startswith("ReferenceWithMeta"):
            import_path = "src.models.cdm.generated.metafields." + camel_to_snake(ref_class)
            type_checking_imports.add(f'from {import_path} import {ref_class}')
            return f'"{ref_class}"'
        elif ref_class in ["LegalEntity", "Party", "AncillaryEntity"]:
            import_path = "src.models.cdm.generated.base.staticdata.party." + camel_to_snake(ref_class)
            type_checking_imports.add(f'from {import_path} import {ref_class}')
            return f'"{ref_class}"'
        elif ref_class in ["Price", "PriceSchedule", "PriceComposite", "CashPrice"]:
            import_path = "src.models.cdm.generated.observable.asset." + camel_to_snake(ref_class)
            type_checking_imports.add(f'from {import_path} import {ref_class}')
            return f'"{ref_class}"'
        elif ref_class == "Key":
    ...
    if prop.items_ref:
        ...
        elif items_class.startswith("ReferenceWithMeta"):
            import_path = "src.models.cdm.generated.metafields." + camel_to_snake(items_class)
            type_checking_imports.add(f'from {import_path} import {items_class}')
            return f'List["{items_class}"]'
        elif items_class in ["LegalEntity", "Party", "AncillaryEntity"]:
            import_path = "src.models.cdm.generated.base.staticdata.party." + camel_to_snake(items_class)
            type_checking_imports.add(f'from {import_path} import {items_class}')
            return f'List["{items_class}"]'
        elif items_class in ["Price", "PriceSchedule", "PriceComposite", "CashPrice"]:
            import_path = "src.models.cdm.generated.observable.asset." + camel_to_snake(items_class)
            type_checking_imports.add(f'from {import_path} import {items_class}')
            return f'List["{items_class}"]'
        elif items_class == "Key":
            import_path = "src.models.cdm.generated.rosetta.model.lib.meta.Key.key"
            type_checking_imports.add(f'from {import_path} import {items_class}')
...
def generate_model_file(schema_name: str, schema: CdmSchema, module_path: List[str], output_dir: Path, circular_deps: Set[Tuple[str, str]]) -> None:
    ...
    # Add properties
    for prop_name, prop in schema.properties.items():
        sanitized_name = camel_to_snake(prop_name)
        ...
        field_def = f'    {sanitized_name}: {prop_type} = Field('
        if prop.required:
            field_def += "default=..."
        else:
            field_def += "None"
