...
def get_import_path(current_module: List[str], target_module: List[str], class_name: str) -> str:
    """Get the correct absolute import path for a class."""
    if not target_module:
        return f"from . import {class_name}"
    
    # Always use direct imports from the specific module file
    module_path = "src.models.cdm.generated"
    if target_module:
        module_path += "." + ".".join(target_module)
...
def determine_cross_import(current_module: List[str], target_module: List[str], class_name: str, circular_deps: Set[Tuple[str, str]]) -> str:
    ...
    if (tuple(current_module), tuple(target_module)) in circular_deps:
        return f"if TYPE_CHECKING:\n    {get_import_path(current_module, target_module, class_name)}"
    
    return get_import_path(current_module, target_module, class_name)
...
def determine_property_type(prop_name: str, prop: CdmProperty, class_name: str, module_path: List[str], type_checking_imports: set, circular_deps: Set[Tuple[str, str]]) -> str:
    ...
    if prop.ref:
        ...
        # Always use string literals for type hints to handle forward references
    ...
    with open(file_path, "w") as f:
        ...
        if type_checking_imports:
            for imp in sorted(type_checking_imports):
                if imp.startswith("if TYPE_CHECKING"):
                    f.write(f"{imp}\n")
            f.write("\n")
            for imp in sorted(type_checking_imports):
                if not imp.startswith("if TYPE_CHECKING"):
...
def update_init_file(module_dir: Path, dependencies: Dict[str, Set[str]], schema_names: List[str]) -> None:
    ...
    py_files = [f for f in module_dir.glob("*.py") if f.is_file() and f.name != "__init__.py"]
    
    # Create ordered list of imports based on dependencies
    ordered_imports = []
    imported = set()
    visiting = set()  # Track nodes being visited to detect cycles
    
    def add_import(schema_name: str):
        if schema_name in imported:
            return
        ...
        if class_name not in imported:
            ...
            for dep in sorted(dependencies.get(schema_name, set())):  # Sort for deterministic order
                add_import(dep)
            visiting.remove(schema_name)
            
            ordered_imports.append(class_name)
            imported.add(class_name)
            imported.add(schema_name)
    
    # Process schemas in sorted order for deterministic output
    for schema_name in sorted(schema_names):
        add_import(schema_name)
    
    # Write __init__.py
    init_path = module_dir / "__init__.py"
    with open(init_path, "w") as f:
        # Write module docstring
        f.write('"""ISDA CDM models."""\n')
        f.write("from typing import TYPE_CHECKING\n\n")
        
        # Write conditional imports for type checking
        f.write("if TYPE_CHECKING:\n")
        for class_name in ordered_imports:
            f.write(f"    from .{camel_to_snake(class_name)} import {class_name}\n")
        f.write("\n")
        
        # Write actual imports
        for class_name in ordered_imports:
            f.write(f"from .{camel_to_snake(class_name)} import {class_name}\n")
        f.write("\n")
        
        # Write model rebuilds
        f.write("# Rebuild models to handle forward references\n")
        for class_name in ordered_imports:
