...
def determine_cross_import(current_module: List[str], target_module: List[str], class_name: str, circular_deps: Set[Tuple[str, str]]) -> str:
    """Determine how to handle cross-module imports, especially for circular dependencies.
    ...
    """
    current_path = '.'.join(current_module)
    target_path = '.'.join(target_module)
    
    # Check if this is a circular dependency
    is_circular = (current_path, target_path) in circular_deps or (target_path, current_path) in circular_deps
    
    import_path = get_import_path(current_module, target_module, class_name)
    
    if is_circular:
        # For circular dependencies, use TYPE_CHECKING conditional import
        return f"if TYPE_CHECKING:\n    from {import_path} import {class_name}"
    else:
        # For non-circular dependencies, use regular import
...
def determine_property_type(prop_name: str, prop: CdmProperty, class_name: str, module_path: List[str], type_checking_imports: set, circular_deps: Set[Tuple[str, str]]) -> str:
    ...
    if prop.ref:
        ...
        # Special handling for MetaFields
        if ref_class == "MetaFields":
            type_checking_imports.add('from src.models.cdm.generated.metafields.meta_fields import MetaFields')
            return "MetaFields"
...
def update_init_file(module_dir: Path, dependencies: Dict[str, Set[str]], schema_names: List[str]) -> None:
    ...
    with open(init_file, "w") as f:
        ...
        # Write conditional imports first
        f.write("if TYPE_CHECKING:\n")
        for schema in schema_names:
            class_name = extract_class_name_from_ref(schema)
            module_path = get_module_path_from_ref(schema)
            if module_path:
                import_path = get_import_path(module_path[:-1], module_path, class_name)
                f.write(f"    from {import_path} import {class_name}\n")
        f.write("\n")
